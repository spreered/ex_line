...
- 每個 API 函式第一參數收 `%ExLine.Client{}`：`ExLine.Messaging.push(client, user_id, messages, opts)`。
...
│
├── ExLine.Messaging             # 發送類 API
│   ├── reply/4  push/4  multicast/4
│   ├── broadcast/3  narrowcast/3  + narrowcast progress
│   ├── validate_*（送出前驗證）
│   ├── *_count / quota / quota_consumption
│   └── display_loading_animation/3  mark_as_read/…
├── ExLine.Content               # 下載媒體 / preview / transcoding status（api-data host）
├── ExLine.Profile               # get_profile/2、followers ids
├── ExLine.Bot                   # bot info
├── ExLine.Group                 # group / room 管理
├── ExLine.RichMenu              # CRUD / per-user / alias / bulk
│
...
| 工作 | 說明 |
|---|---|
| `ExLine.Messaging.reply/push/multicast` | reply/push 核心；multicast 新增 |
...
| `ExLine.Content` | 下載使用者媒體 / preview / transcoding status（走 `api-data.line.me`，client 需可切 host） |
| `ExLine.Profile` | `get_profile/2` + followers ids |
| `ExLine.Bot` | bot info |
...
`ExLine.Audience` / `ExLine.Insight` / `ExLine.Membership` / `ExLine.Coupon` / `ExLine.AccountLink`；beacon/membership 等 webhook event 補完。
